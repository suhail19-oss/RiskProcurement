...
async def calculate_ci_score_from_db(request: Request):
    try:
        ...
        result_data = supplier["cost_subfactors"]
        ...
        update_result = await db.suppliers.update_one(
            ...
            {"$set": {"cost_score": round(cost_efficiency_score, 2)}}  # Update score
        )
        
        if update_result.modified_count == 0:
            raise HTTPException(status_code=500, detail="Failed to update supplier record")
